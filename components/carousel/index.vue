<template>
  <div
    class="carousel"
    :class="{ 'carousel--reverse': paginationPosition === 'top' }"
    @mouseenter="handleMouseCarousel(true)"
    @mouseleave="handleMouseCarousel(false)"
  >
    <div ref="carousel-wrapper" class="carousel-wrapper">
      <div
        ...
        :class="[
          'carousel-inner',
          { 'carousel-inner--center': isCenterModeEnabled },
        ]"
        :style="{
    ...
    <transition name="slide-fade">
      <Navigation
        v-if="navigationEnabled && showNavigation"
        :icon="navigationIcon"
...
export default {
  ...
  mounted() {
    this.showNavigation = this.navigationArrow === 'always'
  ...
  methods: {
    handleMouseCarousel(type) {
      if (this.navigationArrow === 'hover') {
    ...
    onDrag(e) {
      ...
      if (this.rtl) {
        if (Number(this.offset) === 0 && this.dragOffset > 0) {
          this.dragOffset = Math.sqrt(this.resistanceCoef * this.dragOffset)
        } else if (
          Number(this.offset) === this.maxOffset &&
          this.dragOffset < 0
        ) {
          this.dragOffset = -Math.sqrt(-this.resistanceCoef * this.dragOffset)
        }
