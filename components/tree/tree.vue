<template>
  ...
  >
    ...
    <div v-if="isEmpty" class="tree-empty-block">
...
export default {
  ...
  watch: {
    ...
    defaultExpandedKeys(newVal, oldVal) {
      ...
      this.store.setDefaultExpandedKeys(newVal, oldVal)
    },
  ...
  created() {
    ...
    this.$on('tree-node-drag-over', event => {
      ...
      if (oldDropNode && oldDropNode !== dropNode) {
        oldDropNode.$el.classList.remove('is-drop-inner')
      }
  ...
  methods: {
    filter(value) {
      if (!this.filterMethod) {
        throw new Error('[Tree] filterMethod is required when filter')
      }
