...
const TABLE_ROW = [
  { key: 'event', title: '赛事', width: 82 },
  { key: 'time', title: '比赛时间', width: 80 },
  { key: 'vs', title: '对阵', width: 223 },
  { key: 'distance', title: '与本场相隔', width: 96 },
]
...
export default {
  ...
  methods: {
    fetchAllTeamPlan() {
      ...
      Promise.all([
        this.fetchTeamPlan({
          team_id: this.matchInfo.homeInfo.id,
        }),
        this.fetchTeamPlan({
          team_id: this.matchInfo.guestInfo.id,
        }),
      ])
    ...
    fetchDiffDay(Time) {
      const MatchTime = dayjs(Number(this.matchInfo.time))
      const diffDay = diffTime(Time, MatchTime, 'day')
      if (diffDay === 0) {
        const diffHours = diffTime(Time, MatchTime, 'hours')
        if (diffHours === 0) {
          const diffMinute = diffTime(Time, MatchTime, 'minute')
          return `${diffMinute}分`
        } else {
          return `${diffHours}小时`
        }
    ...
    fetchTeamTable(type) {
      ...
      const teamInfo = this.matchInfo[`${type}Info`]
    ...
    fetchTableRow(item, type) {
      const teamId =
        type === 'home'
          ? this.matchInfo.homeInfo.id
          : this.matchInfo.guestInfo.id
