...
export default {
  ...
  computed: {
    ...
    ahList() {
      return this.formatList(this.originData)
    },
  },
  mounted() {
    this.fetchAhList()
  ...
  methods: {
    onSocketMessage() {
      emitter.$on('socket-basketball-ah', this.changeOriginData)
    },
    offSocketMessage() {
      emitter.$off('socket-basketball-ah', this.changeOriginData)
    },
    ...
    fetchAhList() {
      ...
      this.$axios
        .get(api.get.basketball.fetchMatchAh, {
          params: {
