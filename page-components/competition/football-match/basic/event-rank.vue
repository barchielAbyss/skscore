...
export default {
  ...
  computed: {
    homeTeam() {
      const { home_rank: rank, event } = this.matchInfo
      ...
      return {
        ...this.matchInfo.home,
        rank,
        event_name: event.name_js || event.name,
        ...(standings.Home || {}),
        isEmpty: !standings.Home,
      }
    },
    guestTeam() {
      const { guest_rank: rank, event } = this.matchInfo
      let { standings } = this.rankInfo

      standings = standings || {}
      return {
        ...this.matchInfo.guest,
        rank,
        event_name: event.name_js || event.name,
        ...(standings.Away || {}),
  ...
  methods: {
    fetchEventRank() {
      this.loading = true
      this.$axios
        .get(api.get.football.fetchMatchScoreboard, {
          params: {
